...
describe("wind_speed", function()
    ...
    it("windspeed initializes storage when missing", function()
        ...
        assert.is_nil(storage.wind_speed_on_surface[idx])
    ...
    it("windspeed updates and clamps values over multiple steps", function()
        ...
        for _ = 1, max do
            ...
            sum = sum + v
            assert.is_true(v >= 0 and v <= 1, "v should stay in [0,1]")

            local ws = storage.wind_speed_on_surface[idx]

            assert.is_true(ws.v >= 0 and ws.v <= 1, "stored v should stay in [0,1]")
            assert.is_true(ws.delta_v >= -0.05 and ws.delta_v <= 0.05, "delta_v should stay in [-0.05,0.05]")
        end

        --assert.is_true(max * 0.45 <= sum and sum <= max * 0.55, "sum should stay in interval of [45%, 55%]")
        assert.near(max * 0.5, sum, max * 0.1)
    end)
    ...
    it("clamps v even with a large existing delta", function()
        ...
        assert.are.near(v, storage.wind_speed_on_surface[idx].v, 1e-12)

        -- New delta_v is always clamped within [-0.05, 0.05].
        assert.is_true(storage.wind_speed_on_surface[idx].delta_v >= -0.05)
        assert.is_true(storage.wind_speed_on_surface[idx].delta_v <= 0.05)
    end)
end)
